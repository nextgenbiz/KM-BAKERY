...
export default function GlobalSources() {
  return (
    <section className="w-full bg-[#0B2748] py-16">
      <div className="max-w-[1280px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-0 items-center">
        {/* Left copy */}
        <div>
          ...
          <p className="text-slate-300 text-[15px] max-w-md mb-8">
            We source premium products from trusted suppliers in Dubai, Mumbai
            and Delhi, bringing authentic quality to retailers and supermarkets
            across Kerala.
          </p>
...
<Link
  href="/about"
  className="inline-block bg-[#E8402C] text-white font-semibold px-6 py-3 rounded-md hover:bg-[#d0331f] transition-colors"
>
  Know More
</Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {cities.map((c) => (
            <div
              key={c.name}
              className="relative rounded-[2px] overflow-hidden w-full h-[220px] sm:h-[280px]"
            >
              <img
                src={c.img}
                alt={c.name}
